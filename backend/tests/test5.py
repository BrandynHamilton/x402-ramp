...
r = requests.get("http://localhost:8080/sep38/info")
r.raise_for_status()
print(r.json())
...
for asset in assets:
    print(f"- {asset['asset']}")
    if asset.get('asset').startswith('stellar:'):
...
r = requests.get(f"http://localhost:8080/sep38/price", params=params, headers=headers)
r.raise_for_status()
...
r = requests.post(f"http://localhost:8080/sep38/quote", json=params, headers=headers)
r.raise_for_status()
